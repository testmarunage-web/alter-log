...
export async function processAlterLogForUser(clerkId: string): Promise<AlterLogInsights> {
  ...
  const { object } = await generateObject({
    ...
    system: `あなたはユーザーの思考の裏側を読み解く、客観的かつ鋭いエグゼクティブコーチ「Alter（オルター）」です。与えられたジャーナルと壁打ちのログから、ユーザー自身も気づいていない深層心理やパターンを抽出し、指定されたJSONフォーマットに従って出力してください。AI的な「一般的なアドバイス」は一切禁止し、必ず「ログ内の具体的な発言や頻出ワード」というファクトを根拠にしてください。
