...
function buildSystemPrompt(feedbackStyle: string | null): string {
  return `あなたはAlter Log解析エンジンである。ユーザーが書いたジャーナルを、構文・語彙・構造の観点から客観的に分析し、レポートを生成する。

${getBasicStancePrompt(feedbackStyle)}
- 「認知的不協和」ではなく「分かっているのにやめられない矛盾」のように、具体的に言い換える。
...
function buildDailyNotePrompt(feedbackStyle: string | null): string {
  return `あなたはAlter Log解析エンジンである。ユーザーが書いたジャーナルを観察し、daily_noteのみを出力する。

${getDailyNoteStancePrompt(feedbackStyle)}
...
export async function processAlterLogForUser(clerkId: string): Promise<void> {
  ...
  console.log(`[processAlterLogForUser] clerkId=${clerkId} yesterdayJst=${yesterdayJstStr} journalRange=[${jstDayStartUtc.toISOString()}, ${jstDayEndUtc.toISOString()}]`);
  ...
  console.log(`[processAlterLogForUser] journals found=${journals.length} createdAts=[${journals.map((j) => j.createdAt.toISOString()).join(", ")}]`);
  ...
  try {
    const { text } = await generateText({
      ...
      system: buildDailyNotePrompt(user.feedbackStyle),
...
${context}${visionBlock}`;
  ...
  const [objectOutcome, thoughtProfileOutcome] = await Promise.all([
    generateObject({
      ...
      system: buildSystemPrompt(user.feedbackStyle),
      prompt,
    })
...
// ─────────────────────────────────────────────────────────────────────────────
export async function generateForDate(userId: string, targetDate: Date, vision?: string | null, feedbackStyle?: string | null): Promise<"inserted" | "exists" | "no_journals"> {
  ...
  // AlterLog の date = ジャーナルの日付（同日 convention）
  ...
  try {
    const { text } = await generateText({
      model: anthropic("claude-sonnet-4-5"),
      system: buildDailyNotePrompt(feedbackStyle ?? null),
      prompt: `以下のジャーナルを観察し、daily_noteを出力せよ。\n\n${journalBlock}${visionBlock}`,
      maxTokens: 512,
    });
    dailyNote = text.trim() || "INSUFFICIENT_DATA";
...
export async function generateMissingDailyLogs(clerkId: string, maxGenerate = 3): Promise<void> {
  const user = await prisma.user.upsert({
    ...
    create: { clerkId },
    update: {},
    select: { id: true, vision: true, feedbackStyle: true },
  });
  ...
  const missingJournalKeys = [...journalDateKeys].sort().filter((k) => !existingDateKeys.has(k));

  console.log(`[generateMissingDailyLogs] clerkId=${clerkId} userId=${user.id} journalDates=[${[...journalDateKeys].sort().join(",")}] existingDates=[${[...existingDateKeys].join(",")}] missing=[${missingJournalKeys.join(",")}]`);

  let generated = 0;
  for (const key of missingJournalKeys) {
    if (generated >= maxGenerate) break;
    await generateForDate(user.id, new Date(`${key}T00:00:00Z`), user.vision, user.feedbackStyle);
    generated++;
  }
}
